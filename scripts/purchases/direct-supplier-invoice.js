...
const placeInvoiceButton = document.getElementById("place-invoice-button"); 
...
addItemButton.addEventListener("click", () => {
    ...
    const newRowHTML = `
      <tr class="align-right">
          <td>${directInvoiceItems[directInvoiceItems.length - 1].itemCode}</td>
          <td>${directInvoiceItems[directInvoiceItems.length - 1].description}</td>
          <td>${directInvoiceItems[directInvoiceItems.length - 1].quantity}</td>
          <td>
            ${directInvoiceItems[directInvoiceItems.length - 1].unit}
          </td>
          <td>${
            directInvoiceItems[directInvoiceItems.length - 1].requiredDeliveryDate
          }</td>
          <td>${directInvoiceItems[directInvoiceItems.length - 1].priceBeforeTax}</td>
      </tr> 
  `;
...
placeInvoiceButton.addEventListener("click", () => {
    const groupedData = directInvoiceItems.reduce((acc, item) => {
      ...
      post("function/direct-supplier-invoice.php", dataToSend).then((Data) => {
        console.log(Data);
      });
})