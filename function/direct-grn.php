    ...
    foreach ($references as $reference) {
        if (isset($items[$reference])) {
            foreach ($items[$reference] as $item) {
                ...
                $receieveInto = mysqli_real_escape_string($con, $item['receiveInto']);
                ...
                $query = "INSERT INTO service_de_financea.grn_entries (supplier, reference, orderDate, currency, exchangeRate, supplierReference, dimensions, receiveInto, deliverTo, itemCode, description, quantity, unit, requiredDeliveryDate, priceBeforeTax) 
                          VALUES ('$supplier', '$reference', '$orderDate', '$currency', '$exchangeRate', '$supplierReference', '$dimensions', '$receieveInto', '$deliverTo', '$itemCode', '$description', '$quantity', '$unit', '$requiredDeliveryDate', '$priceBeforeTax')";
