    ...
    foreach ($referances as $referance) {
        if (isset($items[$referance])) {
            foreach ($items[$referance] as $item) {
                ...
                $query = "INSERT INTO service_de_financea.direct_invoice (referance, itemCode, description, quantity, unit, priceAfterTax, discount, customer, branch, payment, priceList, date) 
